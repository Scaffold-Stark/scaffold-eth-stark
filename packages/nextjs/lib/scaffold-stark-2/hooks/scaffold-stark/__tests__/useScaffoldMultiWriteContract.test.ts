...
describe.skip("useScaffoldMultiWriteContract Hook", () => {
  ...
  it("should return error if wallet is not connected", async () => {
    ...
    vi.spyOn(result.current, "sendAsync").mockRejectedValue(new Error("Please connect your wallet"));

    await expect(result.current.sendAsync).rejects.toThrowError("Please connect your wallet");
  });
  ...
  it("should handle wrong network", async () => {
    ...
    vi.spyOn(result.current, "sendAsync").mockRejectedValue(new Error("You are on the wrong network"));
  ...
  it("should show error if contract ABI is missing", async () => {
    ...
    vi.spyOn(result.current, "sendAsync").mockRejectedValue(new Error("Function myFunction not found in contract ABI"));

    await expect(result.current.sendAsync).rejects.toThrowError("Function myFunction not found in contract ABI");
  });
  ...
  it("should show error notification if sendAsync is not available", async () => {
    ...
    vi.spyOn(result.current, "sendAsync").mockRejectedValue(new Error("Contract writer error. Try again."));
