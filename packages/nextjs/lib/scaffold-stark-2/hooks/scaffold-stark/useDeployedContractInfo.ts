...
import { Contract, ContractCodeStatus, ContractName, contracts } from "@scaffold-stark-2/utils/scaffold-stark/contract";
...
export const useDeployedContractInfo = <TContractName extends ContractName>(contractName: TContractName) => {
  ...
  useEffect(() => {
    const checkContractDeployment = async () => {
      ...
      try {
        contractClasHash = await publicClient.getClassHashAt(deployedContract.address, "pending" as BlockIdentifier);
      } catch (error) {
