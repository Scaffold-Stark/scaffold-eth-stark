...
import {
  ...
  isCairoTuple,
  parseGenericType,
} from "@scaffold-stark-2/utils/scaffold-stark";
import { replacer } from "@scaffold-stark-2/utils/scaffold-stark/common";
import { AbiOutput } from "@scaffold-stark-2/utils/scaffold-stark/contract";
import { formatEther } from "ethers";
import { CairoCustomEnum, Uint256, getChecksumAddress } from "starknet";

type DisplayContent = Uint256 | string | bigint | boolean | Object | DisplayContent[] | unknown;
...
export const displayTxResult = ({
  ...
  if (functionOutputs !== null && functionOutputs.length !== 0) {
    ...
    if (Array.isArray(parsedParam)) {
      ...
      if (isCairoContractAddress(arrayGenericType)) {
        return JSON.stringify(
          parsedParam.map(addressInBigInt => getChecksumAddress(`0x${addressInBigInt.toString(16)}`)),
        );
      }
      ...
      const displayable = JSON.stringify(
        parsedParam.map(v => {
          ...
          if (typeof txResult === "string") return txResult.replaceAll('"', "").replaceAll("\n", "");
          return txResult;
        }),
        replacer,
      );
    ...
    if (typeof parsedParam === "object" && !Array.isArray(parsedParam)) return JSON.stringify(parsedParam, replacer);
    ...
    if (typeof parsedParam === "bigint") {
      if (parsedParam <= BigInt(Number.MAX_SAFE_INTEGER) && parsedParam >= BigInt(Number.MIN_SAFE_INTEGER)) {
...
const displayTxResultAsText = (displayContent: DisplayContent, contentType?: string) =>
