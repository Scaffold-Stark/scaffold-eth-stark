...
import configExternalContracts from "@scaffold-eth-stark/core/stark/contracts/configExternalContracts";
import deployedContractsData from "@scaffold-eth-stark/core/stark/contracts/deployedContracts";
import predeployedContracts from "@scaffold-eth-stark/core/stark/contracts/predeployedContracts";
import scaffoldConfig from "@scaffold-eth-stark/core/stark/scaffold.config";
import { feltToHex } from "@scaffold-eth-stark/core/stark/utils/scaffold-stark/common";
import {
  isCairoArray,
  isCairoBigInt,
  isCairoBool,
  ...
  isCairoU256,
  parseGenericType,
} from "@scaffold-eth-stark/core/stark/utils/scaffold-stark/types";
...
> = {
  ...
  } & Omit<UseContractWriteProps, "chainId" | "abi" | "address" | "functionName" | "mode"> &
...
export type UseScaffoldArgsParam<
  TContractName extends ContractName,
  TFunctionName extends ExtractAbiFunctionNamesScaffold<ContractAbi<TContractName>>,
> = TFunctionName extends ExtractAbiFunctionNamesWithInputsScaffold<ContractAbi<TContractName>>
  ? {
      args: OptionalTupple<
        UnionToIntersection<
          ExtractArgs<ContractAbi<TContractName>, ExtractAbiFunctionScaffold<ContractAbi<TContractName>, TFunctionName>>
        >
      >;
    }
  : {
...
//@ts-ignore
export function parseParamWithType(paramType: string, param: any, isRead: boolean) {
  if (isRead) {
    if (isCairoTuple(paramType)) {
      ...
      return tryParsingParamReturnObject((x: bigint) => `0x${x.toString(16)}`, param);
    ...
    if (isCairoTuple(paramType)) {
      return tryParsingParamReturnValues(x => stringToObjectTuple(x, paramType), param);
    } else if (isCairoArray(paramType)) {
      const genericType = parseGenericType(paramType)[0];
      ...
      if (typeof param === "string") {
        ...
        if (genericType) {
          ...
          return [
            ...
            ...tokens
              //@ts-ignore
              .map(item => parseParamWithType(genericType, item.trim(), isRead)),
          ];
        } else {
          return param;
        }
      ...
      try {
        if (typeof param.variant == "object" && param.variant !== null) {
          const parsedVariant = Object.keys(param.variant).reduce((acc, key) => {
            ...
            acc[key] = parseParamWithType(param.variant[key].type, param.variant[key].value, false);
            return acc;
          ...
          return Object.keys(param).reduce((acc, key) => {
            const parsed = parseParamWithType(param[key].type, param[key].value, false);
...
export function parseFunctionParams(abiFunction: AbiFunction, inputs: any[], isRead: boolean) {
...
function stringToObjectTuple(tupleString: string, paramType: string): { [key: number]: any } {
